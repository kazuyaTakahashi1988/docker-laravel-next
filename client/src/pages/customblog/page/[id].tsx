  ...
  return (
    <Layout
      /* -------------------------------------------------------
        ▽ 固有 meta ▽
      ---------------------------------------------------------- */
      pageTtl={`Page-${id} | CustomBlog`}
      pageDes={`Page-${id}のディスクリプション`}
      pageUrl={`customblog/page/${id}`}
      // pageKey=""
      // pageThum=""
      pageType="customblog"
    >
      ...
      {/* -------------------------------------------------------
        ▽ 記事一覧  ▽
      ---------------------------------------------------------- */}
      <h2 className="sttl">new CustomBlog - {id}</h2>
      <Articles
        posts={posts}
        slug={`customblog`}
        total={total}
...
export const getStaticPaths = async () => {
  ...
  const paths = pageCount.map((int) => `/customblog/page/${int}`);
...
export const getStaticProps = async (context: { params: any }) => {
  ...
  const res = await fetch(
    `${process.env.MICROCMS_HOST}/api/v1/customblogs?limit=${count}&offset=${offset}&cache=${clear}`, {
    method: "GET",
    headers: {
      "X-MICROCMS-API-KEY": process.env.MICROCMS_API_KEY,
    }
